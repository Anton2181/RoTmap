...
- `ref/` — classic map + old layer images, used only as tracing overlays. Two of them are **realm scans** rather than tracing aids: `Borders_clean.png` (who holds what by right) and `warlords.png` (who holds it now)
...
The terrain and the basemap **bleed half a pixel past their own edges**, because two paths that share an edge do not quite meet: each is rasterised on its own, each covers about half the pixels along the join, and half-coverage laid over half-coverage comes out at three quarters rather than whole. The hex grid used to hide the resulting hairlines, since it strokes every hex outline; with the grid off and the map zoomed out far enough for them to be dense, they read as a shimmer of a grid that isn't there. The geometry is not at fault and there is nothing to close up — every one of the 24,834 shared corners on this map rounds to the same coordinate from both sides — so each patch simply reaches a little past its edge in its own colour. The reach is a **`non-scaling-stroke`**, in screen pixels rather than map ones: the seam is one pixel wide however far out you are zoomed, while a stroke measured in map units would shrink away to nothing at exactly the zoom where the problem shows. Only the layers that tile the whole map need it; the coast fills sit over terrain that is already closed up, so a seam at their edge shows the terrain beneath rather than the background.

Under every layer, and with no switch of its own, is a **basemap**: land and water and nothing else, in the same two colours the terrain palette uses for flat ground and open ocean. It splits at subhex resolution like everything else, so a bay bitten out of a shore hex is water here too. It cannot be turned off because it is what the other layers are drawn *on* — with Terrain off you are reading what a scan or an isochrone says about the ground, and a shape without a shore beside it says nothing about where it is.

The **published sidebar is shorter than the local one**. Several rows exist because they are separate things to *draw*, not separate things to look at: terrain, the subhex fills and the drawn shoreline are one picture (the fills *are* terrain, at subhex resolution, and the line is where they meet), and the two grades of river are one river system at two widths. To a reader those are switches that reveal part of something and leave them wondering where the rest went; to whoever is drawing them they are exactly what needs separating. So on the published map Terrain drives all four coast/terrain groups and one **Rivers** row drives both grades, while locally they all keep their own rows. Only the sidebar changes — the groups keep their separate places in the z-order, which is why one row is handed the others rather than the groups being merged: the coast fills are split around the rivers, and neither that nor the river stacking survives being collapsed into one group. A group folded into another's row can keep its own weight through `op`, which is how the shoreline stays at the grid's 0.28 instead of being pulled up to full strength with the fills.
