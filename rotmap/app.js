...
// A waypoint outline supplies the circular part of a string-tight join. Route points marked with a
// drawing break therefore begin a new SVG subpath rather than drawing a chord or a second arc across
// the marker which is rendered over them immediately afterwards.
function routePathD(pts) {
  return pts.map((p, i) => (i && !p[4] ? 'L' : 'M') + p[0].toFixed(1) + ' ' + p[1].toFixed(1)).join('');
}
...
function landStep(a, b, o, road, crossMajor, bRi, geom) {
  ...
  /* A column marching by night keeps to the road. Off-road it would be marching by daylight, and at
     the night pace that is not a hardship but a *bargain* — 24 miles either way — so a pathfinder
     told only to be quick will leave the road at the first opportunity, march by day, and hand back a
     "night march" with no night in it. In a heatwave that silently gives up the one thing night
     marching is for. So a night leg is solved on roads alone; only when there is no road path at all
...
/* ---------------- marching hard, and what it costs ---------------- */
/* With either pace box ticked the box stops meaning "do this the whole way" and starts meaning "you
   may do this where it pays". Which legs get marched hard is then solved rather than chosen: the
...
/* What one leg costs in morale. Three separate currencies, and they are not interchangeable:
     `checks` are the forced- and night-march rolls, which cost a point of morale on doubles and can
       also fail — these are what the optimiser spends;
     `plain` are rolls that can fail but carry no doubles rider (the Hot weather checks), so they
...
/* ---------------- the morale card ----------------
   One distribution, drawn as bars over the morale the army could finish on, with the allowed floor
   marked. Bars at or above the floor take the accent, bars under it the bad colour — and because
   colour alone is never the whole story, the floor is a labelled rule and the two sides are summed in
   words beneath. A single series, so no legend: the heading names it.

   The headline is the one number the boxes are actually asking about — the chance of finishing at or
   above the floor — set against the certainty asked for, so it reads as met or missed rather than as
   a figure to interpret. */
function moraleCardHTML(o, r) {
  if (!r || r.fail) return '';
  ...
  /* Only a certainty may print as 100%, and only an impossibility as 0. Rounding 99.7% up put
     ...
     between 99.7 and 100 is the whole of what the reader is deciding on. */
  const pct = x => {
    if (x >= 1) return '100%';
    if (x <= 0) return '0%';
    const v = x * 100;
  ...
  while (lo < hi && dist[lo] < 1e-6) lo++;
  while (hi > lo && dist[hi] < 1e-6) hi--;
  const peak = Math.max(...dist);
  const bars = [];
  for (let m = lo; m <= hi; m++) {
    const p = dist[m], h = peak > 0 ? Math.max(p > 0 ? 2 : 0, Math.round(p / peak * 56)) : 0;
    const under = m < o.moraleMin;
    // The mode carries a direct label; the rest would be a number on every bar, which is noise.
    const lbl = p === peak && p > 0 ? `<span class="mval">${pct(p)}</span>` : '';
    bars.push(`<div class="mbar${under ? ' under' : ''}" title="Morale ${m}: ${pct(p)}">` +
              `${lbl}<i style="height:${h}px"></i><b>${m}</b></div>`);
  }
  const met = pAt >= conf;
  const belowP = 1 - pAt;
  const out = walk;
  ...
  return `<div class="moralecard">` +
    ...
    `<div class="mfoot"><span class="k ok"></span>at or above ${o.moraleMin}: ${pct(pAt)}` +
      `<span class="k no"></span>below: ${pct(belowP)}</div>` +
    (mo.relaxed ? `<div class="mwarn">The floor is already out of reach before a hard mile is marched — ` +
                  `showing the gentlest march instead.</div>` : '') +
    `<div class="mfail">Failed checks: <b>${pct(out.anyFail)}</b> chance of at least one` +
      (out.sizeLoss > 0.0005 ? ` · ${(out.sizeLoss * 100).toFixed(1)}% of the army lost on average` : '') +
...
function stepCostAtPace(st, o, was, want) {
  ...
  const noteNight = / \(night\)/.test(note), noteDN = / \(day\+night\)/.test(note);
  const nightOf = m => (m.night !== undefined ? !!m.night : noteNight);
  const dnOf = m => (m.dayNight !== undefined ? !!m.dayNight : noteDN);
  ...
  const at = (m, terrain) => landMilesPerIRL({ road, terrain, forced: m.forced, night: nightOf(m),
                                               dayNight: dnOf(m), liThird: o.liThird,
                                               cavOnly: o.cavOnly, weather: o.weather,
                                               colMiles: o.colMiles });
...
/* Marching after dark is ordered a day at a time in the same way marching hard is, and the leg
   search cannot see that: it chooses the pace a whole leg at a time, and a short leg — or a leg that
   only keeps to the road for part of its length — can come back with a stretch of night marching too
   short to be an order at all. Any run of night steps under a day is put back to daylight here, which
...
function stepDepAtPace(st, o, m) {
  ...
  const mpi = landMilesPerIRL({ road, terrain: st.depTerrain || regionTerrain(st.h, st.ri),
                               forced: m.forced,
                               night: m.night !== undefined ? !!m.night : / \(night\)/.test(note),
                               dayNight: m.dayNight !== undefined ? !!m.dayNight : / \(day\+night\)/.test(note),
                               liThird: o.liThird,
...
/* Pick which steps march hard. `budget` is the number of checks the army can still afford; `nightIrl`
   ...
   choose between. */
/* Which stretches of the march to order at the forced pace.

   An order to force the march is given for **whole IRL days** — one day, two, three; never for the
   two-thirds of a day it takes to cross a hex. So a stretch is not "these hexes" but "from here, for
   N days", and where N days of hard marching runs out is wherever the column has got to, which is
   generally the middle of a hex. That hex is then part forced and part not, and costs the two rates
   in proportion; `frac` carries how much of it was hurried.

   It also makes the budget exact rather than rounded: N whole days of forced marching is N checks,
   with nothing left over to round up. */
function refineForcedSteps(steps, o, budget, nightIrl) {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    cand.push({ i, slow, fast, saved: slow - fast, free });
    fixed += slow;                                   // everything is costed slow, then bought back
  }
  const n = cand.length;
  if (!n) return null;

  const maxDays = Math.max(0, Math.min(budget - Math.ceil(nightIrl - 1e-9), 400));
  if (maxDays < 1) return null;

  /* Every order that could be given: start at `a`, march hard for `d` whole days. Walking forward
     from `a`, each step is taken whole until the day count runs out inside one — that step is split.
     A stretch breaks at a gap in the step numbering, because steps either side of a voyage are not
     one march. `w` is what it costs in checks: nothing when the tradition covers every step of it,
     otherwise the days themselves. */
  const stretch = [];                                // stretch[a] -> [{d, w, save, upto, frac}]
  for (let a = 0; a < n; a++) {
    const opts = [];
    let dur = 0, save = 0, allFree = true, b = a;
    for (let d = 1; d <= maxDays; d++) {
      // extend to cover d whole days
      while (b < n && dur < d - 1e-9) {
        if (b > a && cand[b].i !== cand[b - 1].i + 1) { b = -1; break; }
        dur += cand[b].fast; save += cand[b].saved; allFree = allFree && cand[b].free; b++;
      }
      if (b < 0 || dur < d - 1e-9) break;            // the march runs out before the day does
      // the last step is only partly hurried: give back the overshoot at the difference of the rates
      const over = dur - d, last = cand[b - 1];
      const frac = last.fast > 1e-12 ? 1 - over / last.fast : 1;
      opts.push({ d, w: allFree ? 0 : d, save: save - over / last.fast * last.saved,
                  upto: b - 1, frac: Math.max(0, Math.min(1, frac)) });
    }
    stretch.push(opts);
  }

  /* Best saving from step `a` onward with `v` whole checks left. Backwards, so reading the answer
     back is a plain walk forward. */
  const cap = maxDays;
  const dp = [], pick = [];
  for (let a = 0; a <= n; a++) { dp.push(new Float64Array(cap + 1)); pick.push(new Int32Array(cap + 1).fill(-1)); }
  for (let a = n - 1; a >= 0; a--)
    for (let v = 0; v <= cap; v++) {
      let best = dp[a + 1][v], from = -1;
      const opts = stretch[a];
      for (let k = 0; k < opts.length; k++) {
        const r = opts[k];
        if (r.w > v) continue;
        const gain = r.save + dp[r.upto + 1][v - r.w];
        if (gain > best + 1e-12) { best = gain; from = k; }
      }
      dp[a][v] = best; pick[a][v] = from;
    }

  const frac = new Map();
  let forcedIrl = 0;
  const saved = dp[0][cap];
  for (let a = 0, v = cap; a < n; ) {
    const k = pick[a][v];
    if (k < 0) { a++; continue; }
    const r = stretch[a][k];
    for (let j = a; j <= r.upto; j++) {
      const f = j === r.upto ? r.frac : 1;
      frac.set(cand[j].i, f);
      forcedIrl += f * cand[j].fast;
    }
    v -= r.w; a = r.upto + 1;
  }
  if (!frac.size) return null;
  return { frac, chosen: new Set(frac.keys()), forcedIrl, total: fixed - saved };
}
...
function routeLeg(rt, o) {
  ...
  /* When the paces are being solved for, the DP carries the morale spent so far alongside the ship
     state: two arrangements that reach the same place at the same cost are still different routes if
      ...
      for (let mi = 0; mi < modes.length; mi++) {
        ...
        for (const [stEnd, r] of legs) {
          ...
          /* Arrangements that have already spent more than the army has are kept, but only one of
             ...
             day of daylight marching takes a certain point and no route avoids them all — and an
  ...
  /* The cheapest arrangement that keeps its promise about morale. If none does — the floor is already
     out of reach before a single hard mile is marched — the gentlest march wins instead and says so,
  ...
  if (opt && !best) {
    relaxed = true;
    for (const v of dp.values())
      if (!best || v.checks + v.det < best.checks + best.det ||
  ...
  if (best && opt) {
    ...
    /* Night stretches too short to be an order go back to daylight before anything else is decided,
       ...
       told to make. */
    if (repairShortNightRuns(all, o)) {
      let cost = 0; for (const st of all) cost += st.irl || 0;
      best.cost = cost;
      ...
      for (const st of all) {
        ...
        if (st.paceForced) {
          d0.forcedDay = true;
          if (!marchingCityCovers(o, o.colMiles, /^(road|trade route)/.test(note))) d0.forcedDays += st.irl || 0;
        }
      }
      const t0 = moraleCostOfDays(o, d0);
      best.checks = t0.checks; best.plain = t0.plain; best.det = t0.det;
      best.legCosts = [{ checks: t0.checks, plain: t0.plain, det: t0.det }];
    }
    ...
    const priceChoice = frac => {
      ...
      for (let i = 0; i < all.length; i++) {
        ...
        // the last day of an order.
        const f = frac ? (frac.get(i) ?? 0) : (st.paceFrac ?? (st.paceForced ? 1 : 0));
        const was = st.paceFrac ?? (st.paceForced ? 1 : 0);
        let irl = st.irl || 0, forcedPart = f * irl;
        if (Math.abs(f - was) > 1e-12) {
          const slow = stepCostAtPace(st, o, { forced: !!st.paceForced }, { forced: false });
    ...
    const budgetFor = cost => Math.max(0, moraleCheckBudget(o.morale, best.det, o.moraleMin,
                                        o.moraleConf / 100, o.moraleMax, moraleDrifts(cost), o.moraleRest));
    let ref = null, priced = null;
    /* Every budget from the most the army could stand down to nothing, keeping the **fastest** march
       that keeps its promise — not the first one found. Rest-drift makes the answer non-monotone: a
       big spend can pull the march under twenty days and lose the tick that was repairing a point of
       morale, so a *smaller* spend sometimes finishes sooner. Stopping at the first feasible answer
       let a 70%-certainty route come back slower than the same route asked for 90%. */
    if (o.forced) {
      const top = budgetFor(best.cost);
      for (let bud = top; bud >= 0 && top - bud <= 12; bud--) {
    ...
    if (ref && priced && priced.cost < best.cost - 1e-9) {
      all.forEach((st, i) => {
        const f = priced.fracs[i];
        // A step the order only partly covers is drawn as forced — it was, for part of it — and its
        // cost already carries the two rates in proportion.
  ...
  const ends = new Map();
  for (const v of dp.values()) {
    // Several morale arrangements can end in the same ship state; what a further leg costs from there
    // is the cheapest way of standing there, so the dearer ones are not allowed to overwrite it.
    const ex = ends.get(v.state);
    if (ex === undefined || v.cost < ex) ends.set(v.state, v.cost);
  }
  ...
  // The fourth coordinate is drawing-only provenance. A waypoint is a hard geometric boundary:
  // later lane spreading may meet its ring, but no cosmetic bend rounding may move it. Keep that
  // information on the point itself, since matching by position would also catch a route which only
  // happened to cross some other waypoint's marker.
  const waypointPoint = p => {
    const q = [...p];
    q[3] = 2;
    return q;
  };
  /* A solved feature edge is allowed to run to its road/river node, but a waypoint beside that
     feature is reached where the feature comes closest to the marker. Keeping the rest of the edge
     makes the drawn march pass the marker, carry on to the node, and double back — exactly the
     forty-unit hook that appeared above Iwakhas. Cut at the closest point on the polyline (not merely
     its closest stored sample), then connect that point to the marker. If the feature itself reaches
     the marker to drawing precision, use the marker as the cut point so no tiny, unstable final
     segment can choose the tangent's direction. */
  const featureToWaypoint = (pts, anchor) => {
    if (!pts?.length) return [anchor];
    let best = { i: 0, t: 0, q: pts[0], d2: Infinity };
    for (let i = 0; i + 1 < pts.length; i++) {
      const a = pts[i], b = pts[i + 1], dx = b[0] - a[0], dy = b[1] - a[1];
      const L2 = dx * dx + dy * dy;
      const t = L2 > 1e-12 ? Math.max(0, Math.min(1,
        ((anchor[0] - a[0]) * dx + (anchor[1] - a[1]) * dy) / L2)) : 0;
      const q = [a[0] + dx * t, a[1] + dy * t];
      if (a[2] && b[2]) q[2] = 1;
      const ex = q[0] - anchor[0], ey = q[1] - anchor[1], d2 = ex * ex + ey * ey;
      if (d2 < best.d2 - 1e-9) best = { i, t, q, d2 };
    }
    const out = pts.slice(0, best.i + 1);
    if (best.t > 1e-6) out.push(best.t > 1 - 1e-6 ? pts[best.i + 1] : best.q);
    const last = out[out.length - 1];
    if (best.d2 < 1 && last) out[out.length - 1] = anchor;
    else if (!last || Math.hypot(last[0] - anchor[0], last[1] - anchor[1]) >= 0.01) out.push(anchor);
    else last[3] = 2;
    return out;
  };
  // A leg can contain several solved steps, and the feature which overshoots its destination may
  // belong to the step *before* the final state change. Finish against the whole accumulated leg,
  // not only against whatever geometry the last step happened to carry.
  let legPointStart = 0;
  const finishWaypoint = anchor => {
    const prefix = allPts.slice(0, legPointStart);
    const finished = featureToWaypoint(allPts.slice(legPointStart), anchor);
    allPts.splice(0, allPts.length, ...prefix, ...finished);
  ...
  return { irl: best.cost, hexes: totHex, miles: totMiles, steps, pts: throughSharedEdges(allPts),
           ends, fail: null,
           // What the march cost in morale, and — when the paces were solved for — which legs were
           // marched hard. The readout draws its histogram from these rather than recomputing them.
           morale: { checks: best.checks || 0, det: best.det || 0, plain: best.plain || 0,
                     modes: best.modes || [], legCosts: best.legCosts || [],
                     forcedDays: best.forcedDays,
                     optimised: !!best.optimised, relaxed: !!best.relaxed } };
}
...
/* ---------------- the march as orders: whole days, one order per halt ----------------
   ...
   halt: the march is over, and the day it ends in is spent. Setting out is never one unless that
   waypoint carries an explicit Wait. */
/* The route's box supplies the default; a waypoint action can override it in either direction.
   Absent still means Visit on an ordinary route, preserving saved marches from before actions existed.
   Arrival is handled separately because the march ends there even when the waypoint says Pass. */
const routeStops = rt => rt?.set?.stops !== false;
const waypointWait = w => Number.isInteger(+w?.wait) && +w.wait > 0 ? +w.wait : 0;
function waypointAction(rt, w) {
  if (w?.action === 'pass' || w?.thru) return 'pass';       // `thru` is the old saved form of Pass
  if (w?.action === 'wait' && waypointWait(w)) return 'wait';
  if (w?.action === 'visit') return 'visit';
  return routeStops(rt) ? 'visit' : 'pass';                 // the route switch supplies the default
}
const wpHaltAt = (rt, w) => waypointAction(rt, w) !== 'pass';
const wpHalt = (rt, wi) => wpHaltAt(rt, rt?.wps?.[wi]);
const wpWaitAt = (rt, w) => waypointAction(rt, w) === 'wait' ? waypointWait(w) : 0;
const wpWait = (rt, wi) => wpWaitAt(rt, rt?.wps?.[wi]);
...
function orderedFor(legs, isHalt, waitAt = () => 0, final = true) {
  let waits = waitAt(0), days = waits, pending = 0, exact = 0, halts = waits ? 1 : 0;
  for (let i = 0; i < legs.length; i++) {
    pending += legs[i]; exact += legs[i];
    const last = i === legs.length - 1;
    if ((last && final) || isHalt(i + 1)) {
      days += optDays(pending); pending = 0; halts++;
      const wait = waitAt(i + 1); days += wait; waits += wait;
    }
  }
  return { days, exact, pending, halts, waits, waste: days + pending - exact - waits };
}
...
function orderedOf(rt, r) {
  ...
  return orderedFor(legCosts(rt, r), wi => wpHalt(rt, wi), wi => wpWait(rt, wi));
}
...
function routeDaysTitle(rt, r) {
  const o = orderedOf(rt, r);
  return o ? `${o.days} IRL days ordered — ${o.exact.toFixed(1)} marched` +
    (o.waits ? `, ${o.waits} waited` : '') +
...
function optimiseRouteOrder(rt, mode) {
  ...
  const [af0, sh0] = startState(wps[0].h, wps[0].ri | 0, o);
  push(key(1, 0, af0 * 2 + sh0),
       { days: wpWaitAt(rt, wps[0]), pending: 0, exact: 0, prev: null, at: 0 });
      ...
      for (const st of statesAt(last)) {
        ...
        for (let j = 0; j < n; j++) {
          ...
          for (const [st2, c] of cs[j]) {
            const halt = wpHaltAt(rt, wps[j]), wait = wpWaitAt(rt, wps[j]);
            for (const e of a) {
              const pend = e.pending + c;
              push(key(next, j, st2), halt
                ? { days: e.days + optDays(pend) + wait, pending: 0, exact: e.exact + c, prev: e, at: j }
...
function routeProbeFields() {
  ...
  if (rt.wps.length === 1) {
    ...
    const o = orderedFor(legCosts(rt, r), wi => wpHalt(rt, wi), wi => wpWait(rt, wi), false);
...
/* ---------------- rounding the bends ----------------
   ...
   route that never doubles back is rounded too, and so are the joins the lanes introduce. Waypoint
   centres and the sampled points of their ring arcs carry drawing provenance and come through
   untouched: they are boundary conditions, not another set of bends to smooth. */
...
function fanOutRetraced(pts, R, stops = []) {
  if (pts.length < 3) return pts;
  ...
  for (const p of pts) {
    if (P.length && key(p) === key(P[P.length - 1])) {
      ...
      if (p[3]) P[P.length - 1][3] = Math.max(P[P.length - 1][3] || 0, p[3]);
    } else P.push([...p]);
  }
  // A marker is a circular obstacle for every pass of this route, not only for the occurrence which
  // created it. Otherwise a later waypoint can wrap correctly while an earlier pass through the
  // same coordinate carries straight on beside or through the ring.
  for (const p of P)
    if (stops.some(q => Math.hypot(p[0] - q[0], p[1] - q[1]) < 1)) p[3] = 2;
  /* Feature joins occasionally contain a short fold: A→V walks a few units backwards, then V→B
     retraces exactly past A and continues. It adds no travelled ground to the drawing, but rounding
     the near-180° join turns it into a conspicuous hook. Remove only the geometrically strict case
     where A lies on the following segment, and never remove an actual waypoint reversal. Iterate
     because deleting one fold can expose the same redundant construction immediately beside it. */
  for (let changed = true; changed; ) {
    ...
    for (let i = 1; i + 1 < P.length; i++) {
      ...
      const La = Math.hypot(ax, ay), Lb = Math.hypot(bx, by);
      if (La < 1e-6 || Lb < 1e-6 || La >= Lb) continue;
      const along = (ax * bx + ay * by) / Lb;
      const off = Math.abs(ax * by - ay * bx) / Lb;
      if (along > 0 && along < Lb && off < 1) {
        P.splice(i, 1);
        changed = true;
        break;
      }
  ...
  // Preserve which side each pass intended to occupy. Water safety may close the lanes over the
  // broader crossing, but the short marker-bound tangent still needs that side in order to meet the
  // ring instead of collapsing through its centre.
  const wantedD = [...d];
  for (let i = 0; i < n; i++) {
    if (!d[i]) continue;
  ...
  // A short marker connector often is not itself shared and therefore has no lane rank. Its side is
  ...
  for (let i = 0; i < n; i++) {
    ...
    const hairpin = isRev(i) && re > 0.01 && Math.abs(re - ra) < 0.05
      && ev[0] * av[0] + ev[1] * av[1] < 0;
    const arc = hairpin ? turnArc(V, e, a, u[i - 1]) : null;
    if (arc) {
      // Protect the exact circular join from the generic bend fillet which runs after lane spreading.
      e[3] = 1; a[3] = 1;
      for (const p of arc) p[3] = 1;
      out.push(...arc.slice(1, -1), ...run);
      continue;
    }
    // If incompatible lane directions make a tangent arc impossible, the route must still visit the
    // waypoint. Going through its protected centre is the honest fallback; an ordinary miter can cut
    // the corner outside the ring and was the source of the apparent miss.
    if (V[3] === 2) { out.push(V, ...run); continue; }
...
function arrowsAlong(pts, gap, span, avoid, clear) {
  // Waypoint wraps are deliberate gaps in the route path; the marker outline connects them visually.
  // Measure and place arrows on each real subpath so the invisible chord across a marker contributes
  // neither length nor a misleading arrow direction.
  if (pts.slice(1).some(p => p[4])) {
    const parts = [[]];
    for (const p of pts) {
      if (p[4] && parts.at(-1).length) parts.push([]);
      const q = [...p]; q[4] = 0;
      parts.at(-1).push(q);
    }
    return parts.flatMap(part => arrowsAlong(part, gap, span, avoid, clear));
  }
  const cum = [0];
  for (let i = 0; i + 1 < pts.length; i++)
...
function computeRoute({ preview = false, previewIso = false } = {}) {
  ...
  const departStep = r.steps.find(st => st.dep);
  const depart = departStep?.dep || 0;
  const rows = r.steps.map((st, j) => {
    ...
    const pause = wait ? (() => {
      cum += wait;
      return `<tr class="waitrow"><td></td><td></td><td>wait at waypoint</td><td></td>` +
             `<td>${wait.toFixed(2)}</td><td>${cum.toFixed(1)}</td></tr>`;
    })() : '';
    ...
    // When the paces were solved for, the modes the optimiser chose are the truth about this leg;
    // the hand-marked flags are what the route falls back on when they were not.
    /* The start row is billed for crossing the origin hex, so it is marched at whatever pace the
       column sets out at — and it was the one row that could never say so, because the marks are read
       off the step and the start row is not a step. It borrows the answer from the step that carries
       the crossing, which is the step that actually leaves. Without this, forcing the march from the
       first hex charged the start row the fast rate and then drew the stretch as beginning one row
       below it, which is exactly what it looked like: a click that missed. */
    const paceOf = j === 0 ? departStep : st;
    const om = r.morale?.optimised ? r.morale.modes[paceOf?.leg] : null;
    ...
    const dayNight = night && / \(day\+night\)/.test((paceOf || st).note || '');
  ...
  out.innerHTML =
    ...
    (() => {
      ...
      if (byStep) {
        ...
        if (!marched.some(st => st.paceForced)) return '';
        let runs = 0;
        marched.forEach((st, i) => { if (st.paceForced && !marched[i - 1]?.paceForced) runs++; });
        // Counted in days, because a day is the unit the order is given in.
        const d = Math.round(r.morale?.forcedDays ?? 0);
        return `<div class="fmnote">Forced march: ${d} day${d === 1 ? '' : 's'} in ` +
      ...
      let legs = 0, runs = 0;
      for (let i = 0; i < total; i++) { if (isF(i)) { legs++; if (!isF(i - 1)) runs++; } }
      if (!legs) return '';
      return `<div class="fmnote">Forced march: ${runs} section${runs > 1 ? 's' : ''}, ` +
             `${legs} of ${total} legs — ${om ? 'chosen to fit the morale allowed' : 'right-click a step to change where'}.</div>`;
    })() +
...
function setWaypointAction(ri, wi, action, wait = 0) {
  const rt = S.routes[ri], w = rt?.wps?.[wi];
  if (!w) return;
  wait = action === 'wait' ? wait : 0;
  if (w.action === action && wpWaitAt(rt, w) === wait && !w.thru) return;
  pushUndoRoutes();
  delete w.thru;                              // migrate the old Pass flag when this waypoint is touched
  w.action = action;
  if (wait) w.wait = wait; else delete w.wait;
  computeRoute();
}
...
function stepsToText(ri, simple) {
  ...
  return `${rt.name} — ${ord ? ord.days + ' IRL days ordered (' + r.irl.toFixed(1) + ' marched' + (ord.waits ? ', ' + ord.waits + ' waiting' : '') + ', ' : r.irl.toFixed(1) + ' IRL days ('}` +
...
function routeTip(h, ri) {
  ...
  const base = banked + optDays(pending);
...
for (const id of [...SETTING_NUMS, ...SETTING_CHKS, 'weather'])
...
function hexMenu(h, pt, wp) {
  return box => {
    ...
    if (wp) {
      ...
      const action = waypointAction(rt, w), wait = wpWaitAt(rt, w);
      const actionName = action === 'pass' ? 'Pass' : action === 'wait' ? `Wait ${wait} d` : 'Visit';
      ctxFlyout(ctxItem(box, `Waypoint<span class="arw">${actionName} ▸</span>`), s => {
        const note = (a, text) => `<span class="arw">${text}${a === action ? ' · ✓' : ''}</span>`;
        ctxItem(s, `Pass${note('pass', 'no halt')}`,
                () => { closeCtx(); setWaypointAction(wp.ri, wp.wi, 'pass'); });
        ctxItem(s, `Visit${note('visit', 'halt here')}`,
                () => { closeCtx(); setWaypointAction(wp.ri, wp.wi, 'visit'); });
        const waitItem = ctxItem(s, `Wait <input class="ctxnum" type="number" min="1" step="1" ` +
          `value="${wait || 7}" aria-label="Days to wait"><span class="arw">days${action === 'wait' ? ' · ✓' : ''}</span>`);
        const input = waitItem.querySelector('input');
        const commitWait = () => {
          const days = input.valueAsNumber;
          if (!Number.isInteger(days) || days < 1) {
            toast('Wait must be a positive whole number of IRL days', true);
            input.focus(); input.select();
            return;
          }
          closeCtx();
          setWaypointAction(wp.ri, wp.wi, 'wait', days);
        };
        waitItem.addEventListener('click', e => {
          e.stopPropagation();
          if (e.target !== input) commitWait();
        });
        input.addEventListener('click', e => e.stopPropagation());
        input.addEventListener('change', commitWait);
        input.addEventListener('keydown', e => {
          if (e.key === 'Enter') { e.preventDefault(); commitWait(); }
